    ...
    <section className="relative min-h-screen flex flex-col items-center justify-center px-4 overflow-x-hidden">
      ...
      <div 
        className="absolute w-[140vw] h-[140vw] md:w-[800px] md:h-[800px] rounded-full 
                  bg-gradient-to-r from-blue-500/20 via-cyan-400/20 to-blue-600/15 
                  opacity-80 blur-md transition-all duration-300"
        style={{ 
          perspective: '1200px',
      ...
      >
        ...
        <div className="absolute inset-0 rounded-full overflow-hidden bg-[url('https://upload.wikimedia.org/wikipedia/commons/8/80/World_map_-_low_resolution.svg')] bg-center bg-no-repeat bg-contain opacity-50" />
      ...
      >
        <motion.h1 
          className="text-6xl md:text-8xl font-extrabold mb-6 bg-gradient-to-r from-blue-700 via-blue-500 to-cyan-400 bg-clip-text text-transparent"
          animate={{ 
            textShadow: [
        ...
        <motion.p className="text-xl md:text-2xl text-gray-700 mb-10 font-light">
          {loading ? (
            ...
            >
              Discover the world's <span className="font-semibold text-blue-600">{countryStats.total}</span> countries with comprehensive data
            </motion.span>
          )}
        </motion.p>
        ...
        >
          <div className="relative">
            ...
            <div className="absolute left-0 right-0 mt-2 mb-8 landscape:max-h-[40vh] portrait:max-h-[60vh] overflow-y-auto 
                           bg-white/90 backdrop-blur-md rounded-lg shadow-xl border border-blue-100/50">
        ...
        >
          <Link to="/signup">
            <motion.button
              ...
              className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-blue-700 to-blue-500 text-white rounded-full text-lg font-semibold shadow-lg hover:shadow-blue-500/50 transition-all duration-300"
            >
              Start Exploring
            </motion.button>
          </Link>
          <Link to="/login">
            <motion.button
              ...
              className="w-full sm:w-auto px-8 py-4 bg-white/90 backdrop-blur-sm text-blue-600 border-2 border-blue-200 rounded-full text-lg font-semibold shadow-sm hover:shadow-md transition-all duration-300"
            >
              Sign In
            </motion.button>
          </Link>
        ...
        {/* Enhanced Features Pills with improved animation - MOVED UP to prevent overlap */}
        <motion.div
          className="mt-12 mb-24 flex flex-wrap justify-center gap-3" 
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          {['Flags', 'Maps', 'Capitals', 'Languages', 'Demographics', 'Currencies'].map((feature, i) => (
            <motion.div 
              ...
              className="px-6 py-2.5 bg-white/80 backdrop-blur-sm rounded-full text-sm font-medium text-blue-600 shadow-sm border border-blue-100 hover:shadow-md transition-all"
            >
              {feature}
            </motion.div>
          ))}
      ...
      {/* Improved scroll indicator POSITIONED SEPARATELY at the bottom */}
      <motion.div
